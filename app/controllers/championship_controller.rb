...
class ChampionshipController < ApplicationController
  ...
  def player_list_order_sql(index)
    ...
    when 6 then "IFNULL(SUM(player_games.off_rating), 0) + IFNULL(SUM(player_games.def_rating), 0)"
    when 7 then "IFNULL((IFNULL(SUM(player_games.off_rating), 0) + IFNULL(SUM(player_games.def_rating), 0)) / NULLIF(SUM(player_games.off-player_games.`on`), 0) * 90, 0)"
    when 8 then "IFNULL(SUM(player_games.off_rating), 0)"
    when 9 then "IFNULL(SUM(player_games.def_rating), 0)"
