class HomeController < ApplicationController
  ...
  def index
    ...
    @top_games.each do |g|
      top_quality_championship[[g.phase_id, g.date.in_time_zone(cookie_timezone).to_date]] = g.game_quality if g.game_quality >= top_quality_championship[[g.phase.id, g.date.in_time_zone(cookie_timezone).to_date]].to_f
    end
    @top_games = @top_games.sort_by{|a|[a.date.in_time_zone(cookie_timezone).to_date, -top_quality_championship[[a.phase_id, a.date.in_time_zone(cookie_timezone).to_date]], a.date, -a.game_quality]}
  end
end
