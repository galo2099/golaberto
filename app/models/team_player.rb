class TeamPlayer < ApplicationRecord
  ...
  def self.stats(conditions)
    ...
    PlayerGame.joins("INNER JOIN (#{stats_by_game.to_sql}) g1 on g1.id = player_games.id").joins(game: {phase: :championship}).group("phases.championship_id, team_id, player_id").select("championship_id, player_id, player_games.team_id, player_games.id, SUM(off-`on`) as minutes, SUM(yellow = 1) as yellows, SUM(red = 1) as reds, COUNT(*) as appearances, SUM(IF(off > 0, 1, 0)) as played, SUM(goals) as goals, SUM(own_goals) as own_goals, SUM(penalties) as penalties, SUM(IF(OFF = 0, 1, 0)) as bench, SUM(IF(`on` > 0, 1, 0)) as sub, IFNULL(SUM(off_rating), 0) as off_rating, IFNULL(SUM(def_rating), 0) as def_rating, MAX(date) as latest, player_games.game_id").order("latest DESC")
  end
