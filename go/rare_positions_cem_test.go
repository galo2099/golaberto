...
func TestCEMEvaluationNestedWeightedEvents(t *testing.T) {
	state := CEMEvaluationState{Samples: 3, Work: 300,
		ScoutNear1Probability: 0.02, ScoutNear2Probability: 0.04}
	for _, distance := range []int{0, 1, 2} {
		weight := float64(distance + 1)
		state.Exact.observe(distance == 0, weight)
		state.Near1.observe(distance <= 1, weight)
		state.Near2.observe(distance <= 2, weight)
	}
	updateCEMEvaluationDiagnostics(&state, 1)
	if state.Exact.Hits != 1 || state.Near1.Hits != 2 || state.Near2.Hits != 3 {
		t.Fatalf("nested event hits are wrong: exact=%d near1=%d near2=%d",
			state.Exact.Hits, state.Near1.Hits, state.Near2.Hits)
	}
	if state.Exact.SumY != 1 || state.Near1.SumY != 3 || state.Near2.SumY != 6 ||
		state.Exact.SumY2 != 1 || state.Near1.SumY2 != 5 || state.Near2.SumY2 != 14 {
		t.Fatalf("events did not use identical mixture weights: %+v", state)
	}
}

func TestCEMEvaluationRankingExactTierOutranksSurrogate(t *testing.T) {
	exact := &CEMEvaluationState{Work: 100, Exact: WeightedEventStats{Hits: 1, ESS: 0.1}}
	near := &CEMEvaluationState{Work: 100, Near1EfficiencyRatio: 1e9,
		Near1: WeightedEventStats{ESS: 100}}
	states := []*CEMEvaluationState{near, exact}
	rankCEMEvaluationStates(states)
	if states[0] != exact {
		t.Fatal("an exact held-out event must outrank surrogate evidence")
	}
}

func TestCEMEvaluationRankingUsesNormalizedNearEfficiency(t *testing.T) {
	common := WeightedEventStats{Hits: 2, SumY: 2, SumY2: 2, MaxWeight: 1}
	rare := &CEMEvaluationState{Samples: 100, Work: 100, Near1: common,
		ScoutNear1Probability: 0.01, ScoutNear2Probability: 0.1}
	frequent := &CEMEvaluationState{Samples: 100, Work: 100, Near1: common,
		ScoutNear1Probability: 0.10, ScoutNear2Probability: 0.1}
	updateCEMEvaluationDiagnostics(rare, 1)
	updateCEMEvaluationDiagnostics(frequent, 1)
	if rare.Near1EfficiencyRatio <= frequent.Near1EfficiencyRatio ||
		!cemEvaluationStateBetter(rare, frequent) {
		t.Fatalf("scout normalization did not favor rarer equal-information event: rare=%g frequent=%g",
			rare.Near1EfficiencyRatio, frequent.Near1EfficiencyRatio)
	}
}

func TestCEMEvaluationEarlyAcceptAndOneHitEligibility(t *testing.T) {
	one := &CEMEvaluationState{Exact: WeightedEventStats{Hits: 1, ESS: 1, MaxWeightShare: 1}}
	if cemEvaluationEarlyAccept(one) {
		t.Fatal("one exact event must not stop refinement")
	}
	two := &CEMEvaluationState{Exact: WeightedEventStats{Hits: 2, ESS: 2, MaxWeightShare: 0.5}}
	if !cemEvaluationEarlyAccept(two) {
		t.Fatal("stable repeated exact evidence should stop refinement")
	}
	evaluation := CEMProposalEvaluation{Hits: 1, ESS: 0.5, Work: 100}
	if selectCEMProductionEvaluation([]CEMProposalEvaluation{evaluation}) == nil {
		t.Fatal("one finite exact event must remain eligible at the evaluation cap")
	}
	if selectCEMProductionEvaluation([]CEMProposalEvaluation{{Hits: 0, ESS: 100, Work: 100}}) != nil {
		t.Fatal("near-target information must not bypass the exact-event requirement")
	}
}

func TestCEMEvaluationGlobalSampleCap(t *testing.T) {
	if CEMEvaluationChunkSamples != 100 || CEMMaxEvaluationSamples != 900 ||
		CEMMaxEvaluationSamples%CEMEvaluationChunkSamples != 0 {
		t.Fatalf("invalid chunk/global evaluation limits: chunk=%d maximum=%d",
			CEMEvaluationChunkSamples, CEMMaxEvaluationSamples)
	}
}
