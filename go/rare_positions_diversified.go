...
func runDiversifiedSearchAndProductionDetailed(
	...
	if os.Getenv("RARE_POSITION_LEGACY_PROPOSAL_SEARCH") != "1" {
		adaptiveEstimates, adaptiveDiag, ok := runAdaptivePointStratifiedSearch(group, campaign, table, sortOrder, totalWorkLimit, masterSeed)
		if ok {
			if diagnostics != nil {
