...
func TestSequentialPruningProductionPlannerRespectsGlobalWorkBudget(t *testing.T) {
	plan := planSequentialPruningProduction(2000000, 100, 500, 1000, 2000,
		22, 7, 44, 12, 2, 100000, 0.5)
	...
	if plan.TotalWork != plan.CalibrationWork+plan.TargetWork+plan.AllRankWork || plan.TotalWork > 2000000 {
		t.Fatalf("planned production exceeds or misaccounts budget: %+v", plan)
	}
	if plan.CalibrationWork != 7*500*100 {
		t.Fatalf("calibration work=%d, want 7 fixed 500-sample calibrations", plan.CalibrationWork)
	}
	...
	if capped := planSequentialPruningProduction(2000000, 100, 500, 1000, 2000,
		22, 7, 44, 12, 2, 10, 0.5); !capped.Enabled || capped.TargetSamples > 10 {
		t.Fatalf("hard target sample cap was not applied: %+v", capped)
	}
	if noSpeedup := planSequentialPruningProduction(2000000, 100, 500, 1000, 950,
		22, 7, 44, 12, 2, 100000, 0.5); noSpeedup.Enabled {
	...
	if insufficient := planSequentialPruningProduction(1000, 100, 500, 1000, 2000,
		22, 7, 44, 12, 2, 100000, 0.5); insufficient.Enabled {
...
func TestPrunedSequentialSamplesAreCountedAsZeroObservations(t *testing.T) {
	...
	estimate := runSequentialRareEstimate(base, games, original, components, table, order,
		groups, 1, 0, "original", 1, samples, 99127, "pruned", nil)
...
func TestIndependentAllRankAndSequentialStreamsPoolWithoutReplacingTargetEvidence(t *testing.T) {
	...
	sequential := runSequentialRareEstimate(base, games, original, components, table, order,
		groups, groups[0].Team_id, 0, "original", 1, sequentialN, 902, "sequential_pruned_production", nil)
...
func TestTargetEstimatorsUseSameFixedSampleIndicesAndPruningOnlySavesWork(t *testing.T) {
	...
	for seedIndex := int64(0); seedIndex < 20; seedIndex++ {
		...
		baseline := runSequentialRareEstimate(base, games, original, components, table, order,
			groups, 1, 0, "baseline", 0, samples, seed, "baseline", nil)
		pruned := runSequentialRareEstimate(base, games, original, components, table, order,
			groups, 1, 0, "original", 1, samples, seed, "pruned", nil)
...
func TestSequentialGameOrderingsConstruction(t *testing.T) {
	_, games, _, _, _, _ := pruningFixture(t)
	...
	orderings := buildSequentialGameOrderings(games, 1, 0, normalMeanRanks)
	if len(orderings) != 3 {
		t.Fatalf("want 3 orderings, got %d", len(orderings))
	}
	...
	for _, ord := range orderings {
		names[ord.Name] = true
		if len(ord.GameIndexes) != unplayedCount {
			t.Fatalf("ordering %s index count=%d, want %d", ord.Name, len(ord.GameIndexes), unplayedCount)
		}
		seen := map[int]bool{}
		for _, idx := range ord.GameIndexes {
			if idx < 0 || idx >= len(games) || games[idx].Played || seen[idx] {
				t.Fatalf("ordering %s has invalid/duplicate game index %d", ord.Name, idx)
			}
			seen[idx] = true
		}
	}

	if !names["original"] || !names["target_first"] || !names["target_boundary"] {
		t.Fatalf("missing expected orderings: %+v", names)
	}

	tf := selectGameOrdering(orderings, "target_first")
	sawNonTarget := false
	for _, idx := range tf.GameIndexes {
		g := games[idx]
		isTargetGame := g.HomeId == 1 || g.AwayId == 1
		if sawNonTarget && isTargetGame {
			t.Fatalf("target_first ordering placed target game after non-target game: index %d", idx)
		}
		if !isTargetGame {
			sawNonTarget = true
		}
...
func TestArbitraryCompletionOrderStandingsEquivalence(t *testing.T) {
	base, games, original, groups, table, order := pruningFixture(t)
	...
	const N = 10000
	masterSeed := int64(88123)

	histograms := make(map[string][]int)
	for _, ord := range orderings {
		histograms[ord.Name] = make([]int, len(groups))
		...
		for i := 0; i < N; i++ {
			...
			histograms[ord.Name][rank]++
		}
	}

	origHist := histograms["original"]
	for _, name := range []string{"target_first", "target_boundary"} {
		hist := histograms[name]
		for pos := 0; pos < len(groups); pos++ {
			diff := math.Abs(float64(hist[pos] - origHist[pos]))
			if diff > 250 {
				t.Fatalf("distributional divergence between original and %s at position %d: original=%d %s=%d diff=%.0f",
					name, pos, origHist[pos], name, hist[pos], diff)
			}
		}
	}
}

func TestSequentialPruningSafetyNoFalsePrunes(t *testing.T) {
	base, games, original, groups, table, order := pruningFixture(t)
	shifted := append([]GameProposalMeans(nil), original...)
	for i := range shifted {
		shifted[i].Home *= 1.4
	...
	components := []ProposalComponent{
		{Name: "P", Weight: 0.1, Means: original},
		{Name: "Q", Weight: 0.9, Means: shifted},
	}
	normalMeanRanks := map[int]float64{1: 0.5, 2: 1.5, 3: 2.5, 4: 3.5}

	falsePrunes := 0
	for _, targetTeam := range []int{1, 2, 3, 4} {
		for _, targetPos := range []int{0, 1, 2, 3} {
			orderings := buildSequentialGameOrderings(games, targetTeam, targetPos, normalMeanRanks)
			for _, ord := range orderings {
					...
					if stats.Pruned {
						...
						if fullRank == targetPos {
							...
							t.Errorf("FALSE PRUNE DETECTED! team=%d pos=%d ordering=%s seed=%d: pruned sample actually achieved rank %d",
								targetTeam, targetPos, ord.Name, seed, fullRank)
						}
					}
				}
			}
	...
	if falsePrunes > 0 {
		t.Fatalf("Found %d false prunes during safety testing!", falsePrunes)
	}
}
...
func TestLazyReclassificationRescansAvoided(t *testing.T) {
	...
	normalMeanRanks := map[int]float64{1: 0.5, 2: 1.5, 3: 2.5, 4: 3.5}
	orderings := buildSequentialGameOrderings(games, 1, 0, normalMeanRanks)
	targetBoundary := selectGameOrdering(orderings, "target_boundary")
	...
	if stats.TargetThresholdChanges == 0 {
		t.Fatal("target threshold changes were not tracked")
	}
}
