...
func TestSequentialPruningProductionPlannerRespectsGlobalWorkBudget(t *testing.T) {
	...
	if plan.CalibrationWork != SequentialPruningCalibrationMethods*500*100 {
		t.Fatalf("calibration work=%d, want seven fixed 500-sample calibrations", plan.CalibrationWork)
	}
	...
	if noSpeedup := planSequentialPruningProduction(2000000, 100, 500, 1000, 1029,
...
func TestPrunedSequentialSamplesAreCountedAsZeroObservations(t *testing.T) {
	...
	estimate := runSequentialRareEstimate(base, games, original, components, table, order,
		groups, 1, 0, originalSequentialGameOrdering(games), 1, samples, 99127, "pruned")
...
func TestIndependentAllRankAndSequentialStreamsPoolWithoutReplacingTargetEvidence(t *testing.T) {
	...
	sequential := runSequentialRareEstimate(base, games, original, components, table, order,
		groups, groups[0].Team_id, 0, originalSequentialGameOrdering(games), 1, sequentialN, 902, "sequential_pruned_production")
...
func TestSequentialGameOrderingsAreStaticAndTargetAware(t *testing.T) {
	_, games, _, _, _, _ := pruningFixture(t)
	games[1].Played = true
	orderings := buildSequentialGameOrderings(games, 1, 2,
		map[int]float64{1: 0, 2: 2, 3: 3, 4: 9})
	if len(orderings) != 3 {
		t.Fatalf("got %d orderings, want 3", len(orderings))
	}
	wants := map[string][]int{
		"original":        {0, 2, 3, 4, 5},
		"target_first":    {0, 2, 4, 3, 5},
		"target_boundary": {0, 2, 4, 5, 3},
	}
	for _, ordering := range orderings {
		if !slices.Equal(ordering.GameIndexes, wants[ordering.Name]) {
			t.Errorf("%s indexes=%v want %v", ordering.Name, ordering.GameIndexes, wants[ordering.Name])
		}
	}
}
...
func TestIncrementalPositionBoundsMatchConservativeBoundsOnRandomPrefixes(t *testing.T) {
	...
	for seed := int64(1); seed <= 30; seed++ {
		...
		check(0)
		completionOrder := rng.Perm(len(prefixGames))
		for i, gameIndex := range completionOrder {
			game := prefixGames[gameIndex]
...
func testFullSample(t *testing.T, base []*TeamCampaign, games []*GameType, original []GameProposalMeans,
	...
	targetTeam int, gameIndexes []int, seed int64) (rank int, weight float64, signature uint64) {
	t.Helper()
	rng := rand.New(rand.NewSource(seed))
	chosen := 0
...
func TestSequentialPruningIsZeroContributionAndPreservesPairedSamples(t *testing.T) {
	...
	meanRanks := map[int]float64{1: 2.5, 2: 0, 3: 1, 4: 3}
	for _, ordering := range buildSequentialGameOrderings(games, 1, 0, meanRanks) {
		pruned := 0
		for index := 0; index < 500; index++ {
			seed := targetSampleSeed(9001, index)
			wantRank, wantWeight, wantSignature := testFullSample(t, base, games, original, components, table, order, groups, 1, ordering.GameIndexes, seed)
			rank, weight, stats := simulateTargetSequential(base, games, original, components,
				table, order, groups, 1, 0, ordering.GameIndexes, 1, true, rand.New(rand.NewSource(seed)))
			if stats.Pruned {
				pruned++
				if rank != -1 || weight != 0 || wantRank == 0 {
					t.Fatalf("ordering=%s pruned sample was not a proven zero: stats=%+v rank=%d weight=%g baseline_rank=%d", ordering.Name, stats, rank, weight, wantRank)
				}
				continue
			}
			if rank != wantRank || weight != wantWeight || stats.ScorelineSignature != wantSignature {
				t.Fatalf("ordering=%s unpruned sample differs: got rank=%d weight=%.17g signature=%x want rank=%d weight=%.17g signature=%x",
					ordering.Name, rank, weight, stats.ScorelineSignature, wantRank, wantWeight, wantSignature)
			}
		}
		if pruned == 0 {
			t.Fatalf("ordering=%s produced no mathematically prunable samples", ordering.Name)
		}
	}
}
...
func TestTargetEstimatorsUseSameFixedSampleIndicesAndPruningOnlySavesWork(t *testing.T) {
	...
	for seedIndex := int64(0); seedIndex < 20; seedIndex++ {
		...
		baseline := runSequentialRareEstimate(base, games, original, components, table, order,
			groups, 1, 0, originalSequentialGameOrdering(games), 0, samples, seed, "baseline")
		pruned := runSequentialRareEstimate(base, games, original, components, table, order,
			groups, 1, 0, originalSequentialGameOrdering(games), 1, samples, seed, "pruned")
...
func TestSequentialPruningDoesNotEarlyAcceptGuaranteedEvent(t *testing.T) {
	...
	_, _, stats := simulateTargetSequential(base, games, means,
		...
		1, 0, originalSequentialGameOrdering(games).GameIndexes, 1, false, rand.New(rand.NewSource(3)))
