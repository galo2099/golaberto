...
const (
	...
	SequentialPruningRuntimeMargin               = 1.03
	SequentialPruningCalibrationMethods          = 7
)
...
// SequentialGameOrdering is built once per target and contains original game
// indexes. Proposal mean arrays deliberately remain in fixture order.
type SequentialGameOrdering struct {
	Name        string
	GameIndexes []int
}

func originalSequentialGameOrdering(games []*GameType) SequentialGameOrdering {
	ordering := SequentialGameOrdering{Name: "original"}
	for gameIndex, game := range games {
		if !game.Played {
			ordering.GameIndexes = append(ordering.GameIndexes, gameIndex)
		}
	}
	return ordering
}

func buildSequentialGameOrderings(games []*GameType, targetTeamID, targetPosition int,
	normalMeanRanks map[int]float64) []SequentialGameOrdering {
	original := SequentialGameOrdering{Name: "original"}
	targetFirst := SequentialGameOrdering{Name: "target_first"}
	targetBoundary := SequentialGameOrdering{Name: "target_boundary"}
	var nonTarget, twoBoundary, oneBoundary, remaining []int
	for gameIndex, game := range games {
		if game.Played {
			continue
		}
		original.GameIndexes = append(original.GameIndexes, gameIndex)
		if game.HomeId == targetTeamID || game.AwayId == targetTeamID {
			targetFirst.GameIndexes = append(targetFirst.GameIndexes, gameIndex)
			targetBoundary.GameIndexes = append(targetBoundary.GameIndexes, gameIndex)
			continue
		}
		nonTarget = append(nonTarget, gameIndex)
		homeNear := math.Abs(normalMeanRanks[game.HomeId]-float64(targetPosition)) <= 3
		awayNear := math.Abs(normalMeanRanks[game.AwayId]-float64(targetPosition)) <= 3
		switch {
		case homeNear && awayNear:
			twoBoundary = append(twoBoundary, gameIndex)
		case homeNear || awayNear:
			oneBoundary = append(oneBoundary, gameIndex)
		default:
			remaining = append(remaining, gameIndex)
		}
	}
	targetFirst.GameIndexes = append(targetFirst.GameIndexes, nonTarget...)
	targetBoundary.GameIndexes = append(targetBoundary.GameIndexes, twoBoundary...)
	targetBoundary.GameIndexes = append(targetBoundary.GameIndexes, oneBoundary...)
	targetBoundary.GameIndexes = append(targetBoundary.GameIndexes, remaining...)
	return []SequentialGameOrdering{original, targetFirst, targetBoundary}
}
...
type SequentialPruningCalibrationSummary struct {
	...
	PruneP10         int     `json:"prune_p10"`
	PruneP25         int     `json:"prune_p25"`
	PruneP50         int     `json:"prune_p50"`
	PruneP75         int     `json:"prune_p75"`
	PruneP90         int     `json:"prune_p90"`
	PrunedBefore25   float64 `json:"pruned_before_25pct"`
	PrunedBefore50   float64 `json:"pruned_before_50pct"`
	PrunedBefore75   float64 `json:"pruned_before_75pct"`
}
...
func planSequentialPruningProduction(remainingWork, allRankWorkPerSample int64,
	...
	if remainingWork <= 0 || allRankWorkPerSample <= 0 || desiredCalibrationSamples <= 0 ||
		baselineSamplesPerSecond <= 0 || prunedSamplesPerSecond < baselineSamplesPerSecond*SequentialPruningRuntimeMargin ||
	...
	// Calibration compares baseline plus every fixed ordering/stride candidate.
	maxCalibrationSamples := int(remainingWork / (5 * SequentialPruningCalibrationMethods * allRankWorkPerSample))
	calibrationSamples := minInt(desiredCalibrationSamples, maxCalibrationSamples)
	if calibrationSamples <= 0 {
		return plan
	}
	plan.CalibrationSamples = calibrationSamples
	plan.CalibrationWork = int64(calibrationSamples*SequentialPruningCalibrationMethods) * allRankWorkPerSample
...
func calibrateSequentialPruning(baseCampaign []*TeamCampaign, games []*GameType,
	...
	orderings []SequentialGameOrdering, samples int, masterSeed int64) (SequentialGameOrdering, int, []SequentialPruningCalibrationSummary) {
	unplayed := 0
	for _, game := range games {
		if !game.Played {
			unplayed++
	...
	results := make([]SequentialPruningCalibrationSummary, 0, SequentialPruningCalibrationMethods)
	run := func(method string, ordering SequentialGameOrdering, stride int) SequentialPruningCalibrationSummary {
		result := SequentialPruningCalibrationSummary{Method: method, Ordering: ordering.Name, Stride: stride, Samples: samples}
		stream := deriveRarePositionSeed(masterSeed, fmt.Sprintf("runtime-calibration-%s-%s-%d", method, ordering.Name, stride))
		start := time.Now()
		...
		for i := 0; i < samples; i++ {
			...
			if method == "baseline" {
				...
				_, _, stats := simulateTargetSequential(baseCampaign, games, originalMeans, components,
					table, sortOrder, teamGroups, targetTeamID, targetPosition, ordering.GameIndexes, stride, false, rng)
		...
		populatePruneTiming(&result, pruneGames, unplayed)
		return result
	}
	results = append(results, run("baseline", SequentialGameOrdering{Name: "baseline_full"}, 0))
	bestOrdering, bestStride, bestRate := orderings[0], 4, -1.0
	for _, ordering := range orderings {
		for _, stride := range []int{4, 8} {
			result := run("pruned", ordering, stride)
			results = append(results, result)
			if result.SamplesPerSecond > bestRate {
				bestOrdering, bestStride, bestRate = ordering, stride, result.SamplesPerSecond
			}
		}
	}
...
func percentile(sortedValues []int, percent int) int {
	...
	index := int(math.Ceil(float64(percent)*float64(len(sortedValues))/100.0)) - 1
	if index < 0 {
		index = 0
	}
...
func populatePruneTiming(result *SequentialPruningCalibrationSummary, pruneGames []int, unplayed int) {
	if len(pruneGames) == 0 {
		return
	}
	sort.Ints(pruneGames)
	result.PruneP10, result.PruneP25 = percentile(pruneGames, 10), percentile(pruneGames, 25)
	result.PruneP50, result.PruneP75 = percentile(pruneGames, 50), percentile(pruneGames, 75)
	result.PruneP90 = percentile(pruneGames, 90)
	for _, completed := range pruneGames {
		if completed*4 <= unplayed {
			result.PrunedBefore25++
		}
		if completed*2 <= unplayed {
			result.PrunedBefore50++
		}
		if completed*4 <= unplayed*3 {
			result.PrunedBefore75++
		}
	}
	denominator := float64(len(pruneGames))
	result.PrunedBefore25 /= denominator
	result.PrunedBefore50 /= denominator
	result.PrunedBefore75 /= denominator
}

// simulateTargetSequential uses the same component draw, per-game distributions, Poisson
...
func simulateTargetSequential(
	...
	sortOrder []SortType,
	teamGroups []TeamType,
	targetTeamID, targetPosition int, orderedGameIndexes []int,
	stride int, trackScoreline bool,
	rng *rand.Rand,
...
func runSequentialRareEstimate(
	...
	targetTeamID, targetPosition int, ordering SequentialGameOrdering, stride int, samples int, masterSeed int64, method string,
) SequentialRareEstimate {
	result := SequentialRareEstimate{TeamID: targetTeamID, Position: targetPosition, Seed: masterSeed,
		Method: method, Ordering: ordering.Name, Stride: stride, Samples: samples}
	if samples <= 0 {
		return result
	}
	unplayed := 0
	...
	if result.Pruned > 0 {
		...
		timing := SequentialPruningCalibrationSummary{}
		populatePruneTiming(&timing, pruneGames, unplayed)
		result.PruneP10, result.PruneP25, result.PruneP50 = timing.PruneP10, timing.PruneP25, timing.PruneP50
		result.PruneP75, result.PruneP90 = timing.PruneP75, timing.PruneP90
		result.PrunedBefore25, result.PrunedBefore50, result.PrunedBefore75 = timing.PrunedBefore25, timing.PrunedBefore50, timing.PrunedBefore75
	}
	if result.RawHits > 0 {
