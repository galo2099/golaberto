...
func TestSequentialRarePositionEqualRuntimeBenchmark(t *testing.T) {
	...
	for _, seed := range seeds {
		...
		sequential := runSequentialRareEstimate(base, games, original, components, table, order,
			group.Team_groups, teamID, position, originalSequentialGameOrdering(games), bestStride, prunedSamples, sequentialSeed, "pruned")
	...
	for i := 0; i < samples; i++ {
		...
		if method == "baseline" {
			...
			_, _, stats := simulateTargetSequential(base, games, original, components, table,
				order, groups, team, position, originalSequentialGameOrdering(games).GameIndexes, stride, false, rng)
