...
func runRarePositionSearchEvaluationProduction(group *GroupType, campaign []*TeamCampaign,
	...
	if sequentialPruningEnabled() && design.Kind == "importance_sampling" {
		calibrationSamples := minInt(sequentialPruningCalibrationSampleCount(),
			int(remainingWork/(35*productionISWorkPerSample)))
		if calibrationSamples > 0 {
			sequentialOrdering, sequentialStride, sequentialCalibration = calibrateSequentialPruning(
				...
				group.Team_groups, design.TargetTeam, design.TargetPosition, normalMeanRanks,
				calibrationSamples,
			...
			for _, calibration := range sequentialCalibration {
				...
				if calibration.Method == "baseline" || calibration.Ordering == "baseline" {
					baselineRate = calibration.SamplesPerSecond
				}
				if calibration.Ordering == sequentialOrdering && calibration.Stride == sequentialStride {
			...
			sequentialPlan = planSequentialPruningProduction(remainingWork, productionISWorkPerSample,
				calibrationSamples, baselineRate, prunedRate, averageGames, len(sequentialCalibration),
				unplayedGames, numTeams, len(design.Components), sequentialPruningTargetSampleCap(),
				sequentialCalibrationSeconds)
			sequentialPlan.SelectedOrdering = sequentialOrdering
			sequentialPlan.SelectedStride = sequentialStride
			if sequentialOrdering != "" && sequentialStride > 0 && sequentialPlan.Enabled {
				design.Samples = sequentialPlan.AllRankSamples
				design.Work = sequentialPlan.AllRankWork
				sequentialCalibrationWork = sequentialPlan.CalibrationWork
				log.Printf("rare-position-sequential-pruning-calibration: group=%d target_team=%d target_position=%d selected_ordering=%s selected_stride=%d calibration_samples_per_method=%d calibration_work=%d baseline_seconds_per_sample=%.8g pruned_seconds_per_sample=%.8g speedup=%.4f runtime_fraction_target=%.2f expected_all_rank_seconds=%.4f expected_target_seconds=%.4f expected_total_seconds=%.4f target_samples=%d target_nominal_work=%d target_work_per_sample=%d all_rank_samples=%d all_rank_work=%d total_production_work=%d budget=%d calibration=%+v",
					group.Id, design.TargetTeam, design.TargetPosition, sequentialOrdering, sequentialStride,
				...
				sequentialPlan.Enabled = false
				sequentialCalibrationWork = int64(calibrationSamples*len(sequentialCalibration)) * productionISWorkPerSample
				remainingAfterCalibration := remainingWork - sequentialCalibrationWork
				if remainingAfterCalibration > 0 {
				...
				log.Printf("rare-position-sequential-pruning-disabled: group=%d reason=no_speedup_or_plan_unaffordable remaining_work=%d work_per_sample=%d calibration=%+v",
	...
	log.Printf("rare-position-design-freeze: group=%d design=%s target_team=%d target_position=%d selected_snapshot_iteration=%d components=%s all_rank_samples=%d all_rank_nominal_work=%d target_samples=%d target_nominal_work=%d calibration_nominal_work=%d total_nominal_work=%d sequential_pruning=%t sequential_ordering=%s sequential_stride=%d baseline_seconds_per_sample=%.8g pruned_seconds_per_sample=%.8g speedup=%.4f runtime_fraction_target=%.2f expected_all_rank_seconds=%.4f expected_target_seconds=%.4f expected_total_seconds=%.4f remaining_work_before_production=%d",
		...
		design.Work+sequentialPlan.TargetWork+sequentialCalibrationWork, sequentialPlan.Enabled,
		sequentialOrdering, sequentialStride,
		sequentialPlan.BaselineSecondsPerSample, sequentialPlan.PrunedSecondsPerSample,
		sequentialPlan.Speedup, sequentialPlan.RuntimeFractionTarget,
	...
	if sequentialPlan.Enabled {
		...
		estimate := runSequentialRareEstimate(campaign, group.Games, originalMeans, design.Components,
			...
			sequentialOrdering, sequentialStride, sequentialPlan.TargetSamples, targetSeed,
			"sequential_pruned_production", normalMeanRanks)
		...
		log.Printf("rare-position-sequential-pruning-production: group=%d team=%d position=%d ordering=%s stride=%d samples=%d pruned=%d pruned_fraction=%.4f games=%d average_games=%.2f prune_p10=%.1f prune_p25=%.1f prune_p50=%.1f prune_p75=%.1f prune_p90=%.1f pruned_before_25pct=%d pruned_before_50pct=%d pruned_before_75pct=%d solver_checks=%d solver_seconds=%.6f solver_fraction=%.4f elapsed_seconds=%.6f samples_per_second=%.4f hits=%d ESS=%.3f ESS_per_second=%.4g p=%.8g se=%.3g relSE=%.3f nominal_work=%d",
