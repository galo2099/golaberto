...
func runRarePositionSearchEvaluationProduction(group *GroupType, campaign []*TeamCampaign,
	...
	if cap := int64(CEMMaxEvaluationSamples) * estimateSeasonWork(unplayedGames, 2, numTeams); evaluationWorkLimit > cap {
		evaluationWorkLimit = cap
	}
	...
	evaluationWorkPerSnapshot := int64(CEMEvaluationChunkSamples) * evaluationWorkPerSample
	...
	raceBudget := evaluationWorkRemaining
	if remainingWork < raceBudget {
		raceBudget = remainingWork
	}
	evaluationStates, selectedEvaluation, evaluationWork, evaluationEarlyAccept := raceCEMEvaluationSnapshots(
		evaluationCandidates, normalPositionCounts, normalSamples, originalMeans, campaign,
		group.Games, table, sortOrder, group.Team_groups, raceBudget, evaluationWorkPerSample,
		plainWorkPerSample, evaluationSeed, group.Id)
	evaluationWorkRemaining -= evaluationWork
	remainingWork -= evaluationWork
	evaluations := make([]CEMProposalEvaluation, 0, len(evaluationStates))
	for _, state := range evaluationStates {
		evaluations = append(evaluations, CEMProposalEvaluation{Snapshot: state.Snapshot,
			Samples: state.Samples, Hits: state.Exact.Hits, SumY: state.Exact.SumY,
			SumY2: state.Exact.SumY2, Probability: state.Exact.Probability,
			StdErr: state.Exact.StdErr, RelSE: state.Exact.RelSE, ESS: state.Exact.ESS,
			ESSPerWork:          weightedESSPerWork(state.Exact, state.Work),
			MaxEventWeightShare: state.Exact.MaxWeightShare, Work: state.Work})
	}
	selectionReason := "no_evaluated_snapshot_with_exact_event"
	if selectedEvaluation != nil {
		selectionReason = "racing_winner_with_exact_event"
	}
	...
	if selectedEvaluation != nil {
		var selectedState *CEMEvaluationState
		for _, state := range evaluationStates {
			if state.Snapshot.CandidateTeam == selectedEvaluation.Snapshot.CandidateTeam && state.Snapshot.CandidatePosition == selectedEvaluation.Snapshot.CandidatePosition && state.Snapshot.SourceIteration == selectedEvaluation.Snapshot.SourceIteration {
				selectedState = state
				break
			}
		}
		log.Printf("rare-position-evaluation-result: group=%d selected=importance_sampling team=%d position=%d snapshot_iteration=%d evaluation_samples=%d evaluation_work=%d exact_hits=%d exact_ess=%.3f exact_ess_per_work=%.8g near1_ess=%.3f near1_efficiency_ratio=%.3f near2_ess=%.3f near2_efficiency_ratio=%.3f early_accept=%t reason=%s",
			group.Id, selectedEvaluation.Snapshot.CandidateTeam, selectedEvaluation.Snapshot.CandidatePosition,
			selectedEvaluation.Snapshot.SourceIteration, selectedEvaluation.Samples, evaluationWork,
			selectedEvaluation.Hits, selectedEvaluation.ESS, selectedEvaluation.ESSPerWork,
			selectedState.Near1.ESS, selectedState.Near1EfficiencyRatio, selectedState.Near2.ESS,
			selectedState.Near2EfficiencyRatio, evaluationEarlyAccept, selectionReason)
	} else {
		log.Printf("rare-position-evaluation-result: group=%d selected=plain_mc team=0 position=-1 snapshot_iteration=0 evaluation_samples=0 evaluation_work=%d exact_hits=0 exact_ess=0 exact_ess_per_work=0 near1_ess=0 near1_efficiency_ratio=0 near2_ess=0 near2_efficiency_ratio=0 early_accept=false reason=%s", group.Id, evaluationWork, selectionReason)
	}
