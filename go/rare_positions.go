...
func runRarePositionSearchEvaluationProduction(group *GroupType, campaign []*TeamCampaign,
	...
	sequentialStride := 0
	sequentialOrdering := SequentialGameOrdering{Name: "none"}
	if sequentialPruningEnabled() && design.Kind == "importance_sampling" {
		calibrationSamples := minInt(sequentialPruningCalibrationSampleCount(),
			int(remainingWork/(5*SequentialPruningCalibrationMethods*productionISWorkPerSample)))
		if calibrationSamples > 0 {
			sequentialCalibrationWork = int64(calibrationSamples*SequentialPruningCalibrationMethods) * productionISWorkPerSample
			normalMeanRanks := make(map[int]float64, len(teamSearches))
			for teamID, search := range teamSearches {
				normalMeanRanks[teamID] = search.NormalMeanRank
			}
			orderings := buildSequentialGameOrderings(group.Games, design.TargetTeam, design.TargetPosition, normalMeanRanks)
			sequentialOrdering, sequentialStride, sequentialCalibration = calibrateSequentialPruning(
			...
			for _, calibration := range sequentialCalibration {
				...
				log.Printf("rare-position-sequential-pruning-candidate: group=%d target_team=%d target_position=%d ordering=%s stride=%d samples=%d elapsed_seconds=%.6f samples_per_second=%.4f average_games=%.2f pruned_fraction=%.4f solver_checks=%d solver_seconds=%.6f solver_fraction=%.4f prune_p10=%d prune_p25=%d prune_p50=%d prune_p75=%d prune_p90=%d pruned_before_25pct=%.4f pruned_before_50pct=%.4f pruned_before_75pct=%.4f",
					group.Id, design.TargetTeam, design.TargetPosition, calibration.Ordering,
					calibration.Stride, calibration.Samples, calibration.ElapsedSeconds,
					calibration.SamplesPerSecond, calibration.AverageGames, calibration.PrunedFraction,
					calibration.SolverChecks, calibration.SolverSeconds, calibration.SolverFraction,
					calibration.PruneP10, calibration.PruneP25, calibration.PruneP50,
					calibration.PruneP75, calibration.PruneP90, calibration.PrunedBefore25,
					calibration.PrunedBefore50, calibration.PrunedBefore75)
				if calibration.Method == "baseline" {
					baselineRate = calibration.SamplesPerSecond
				}
				if calibration.Method == "pruned" && calibration.Ordering == sequentialOrdering.Name && calibration.Stride == sequentialStride {
			...
			if sequentialPlan.Enabled {
				...
				reason := "runtime_plan_unaffordable"
				if baselineRate > 0 && prunedRate < baselineRate*SequentialPruningRuntimeMargin {
					reason = "no_runtime_speedup"
				}
				log.Printf("rare-position-sequential-pruning-disabled: group=%d reason=%s required_speedup=%.2f measured_speedup=%.4f remaining_work=%d work_per_sample=%d calibration=%+v",
					group.Id, reason, SequentialPruningRuntimeMargin, prunedRate/baselineRate,
					remainingWork, productionISWorkPerSample, sequentialCalibration)
			}
		} else {
	...
	log.Printf("rare-position-design-freeze: group=%d design=%s target_team=%d target_position=%d selected_snapshot_iteration=%d components=%s all_rank_samples=%d all_rank_nominal_work=%d target_samples=%d target_nominal_work=%d calibration_nominal_work=%d total_nominal_work=%d sequential_pruning=%t sequential_ordering=%s sequential_stride=%d baseline_seconds_per_sample=%.8g pruned_seconds_per_sample=%.8g speedup=%.4f runtime_fraction_target=%.2f expected_all_rank_seconds=%.4f expected_target_seconds=%.4f expected_total_seconds=%.4f remaining_work_before_production=%d",
		...
		design.Work+sequentialPlan.TargetWork+sequentialCalibrationWork, sequentialPlan.Enabled, sequentialOrdering.Name, sequentialStride,
		sequentialPlan.BaselineSecondsPerSample, sequentialPlan.PrunedSecondsPerSample,
		sequentialPlan.Speedup, sequentialPlan.RuntimeFractionTarget,
	...
	if sequentialPlan.Enabled {
		...
		estimate := runSequentialRareEstimate(campaign, group.Games, originalMeans, design.Components,
			...
			sequentialOrdering, sequentialStride, sequentialPlan.TargetSamples, targetSeed, "sequential_pruned_production")
		...
		log.Printf("rare-position-sequential-pruning-production: group=%d team=%d position=%d ordering=%s stride=%d samples=%d pruned=%d pruned_fraction=%.4f games=%d average_games=%.2f prune_p10=%d prune_p25=%d prune_p50=%d prune_p75=%d prune_p90=%d pruned_before_25pct=%.4f pruned_before_50pct=%.4f pruned_before_75pct=%.4f solver_checks=%d solver_seconds=%.6f solver_fraction=%.4f elapsed_seconds=%.6f samples_per_second=%.4f hits=%d ESS=%.3f ESS_per_second=%.4g p=%.8g se=%.3g relSE=%.3f nominal_work=%d",
			group.Id, design.TargetTeam, design.TargetPosition, sequentialOrdering.Name, sequentialStride, estimate.Samples,
			...
			estimate.PrunedBefore25, estimate.PrunedBefore50, estimate.PrunedBefore75,
