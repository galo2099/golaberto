...
func simulatePointHybridSeasons(
	...
	var universe *PointStratumUniverse
	if stratum != nil {
	...
	for sample := 0; sample < samples; sample++ {
		...
		if conditional && stratum != nil && !stratum.contains(observedCode) {
		...
		for rank, c := range sorted[:idx] {
			...
			if universe != nil && c.id == universe.TargetTeam && !conditional && stratum != nil && !stratum.contains(observedCode) {
				complement[rank]++
			}
		}
	}
