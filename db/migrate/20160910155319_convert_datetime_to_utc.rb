class ConvertDatetimeToUtc < ActiveRecord::Migration
  TZ_CHAMP = { ActiveSupport::TimeZone.new("Europe/London") => Championship.where("name LIKE 'England%' OR name = 'Dutch Eredivisie' OR name = 'France Ligue 1' OR name = 'Scotland Premier'").map(&:id),
               ...
               ActiveSupport::TimeZone.new("Europe/Zurich") => Championship.where("name = 'Champions League' OR name = 'Europa League' OR name = 'UEFA Cup'").map(&:id),
  ...
  def up
    [[:games, Game], [:game_versions, Game::Version]].each do |g|
      TZ_CHAMP.each do |tz, champs|
        say_with_time "Updating games for #{tz.name}" do
          ...
          while first < last
            g[1].joins(phase: :championship).where("championships.id in (?)", champs).where(has_time: true).order(:date).where("date >= ? and date <= ?", tz.period_for_local(first).start_transition.try(:at).try(:to_datetime) || first, (tz.period_for_local(first).end_transition.try(:at).try(:to_datetime) || "2050-01-01".to_datetime)).update_all("date = TIMESTAMPADD(SECOND, -#{tz.period_for_local(first).offset.utc_total_offset}, date)")
            first = (tz.period_for_local(first).end_transition.try(:at).try(:to_datetime) || "2050-01-01".to_datetime) + 2.hours
          end
        end
      end
    end
    ...
    [[:games, Game], [:game_versions, Game::Version]].each do |g|
      TZ_CHAMP.each do |tz, champs|
        say_with_time "Updating games for #{tz.name}" do
          ...
          last = g[1].joins(phase: :championship).where("championships.id in (?)", champs).where(has_time: true).order(:date).last.date
          while first < last
            g[1].joins(phase: :championship).where("championships.id in (?)", champs).where(has_time: true).order(:date).where("date >= ? and date =< ?", tz.period_for_utc(first).start_transition.try(:at).try(:to_datetime) || first, (tz.period_for_utc(first).end_transition.try(:at).try(:to_datetime) || "2050-01-01".to_datetime)).update_all("date = TIMESTAMPADD(SECOND, #{tz.period_for_utc(first).offset.utc_total_offset}, date)")
            first = (tz.period_for_utc(first).end_transition.try(:at).try(:to_datetime) || "2050-01-01".to_datetime) + 2.hours
          end
        end
      end
    end
