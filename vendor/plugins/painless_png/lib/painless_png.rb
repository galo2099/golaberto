...
module ActionView::Helpers::AssetTagHelper
  ...
  def image_tag(source, options={})
    ...
    # ADDED:
    styles[:width] = "#{styles[:width]}px" if not styles[:width] =~ /\.*px$/
    styles[:height] = "#{styles[:height]}px" if not styles[:height] =~ /\.*px$/


    # This is the cornerstone of the hack: apply the propriety alpha transform filter
    styles[:filter] = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='#{asset_path}')"
    # REMOVED   , sizingMethod='scale'


    styles_string = styles.map { |key, value| %(#{key}:#{value}) }.join(";")
    options[:style] = styles_string + (options[:style] ? "; #{options[:style]}" : "")
    options[:src] = '/images/blank.gif' # NEW
    # Place the filter in a div

    #content_tag('div', nil, options) # REPLACED by:
