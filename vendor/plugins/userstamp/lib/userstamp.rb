  ...
  module Userstamp
    ...
    def create_with_user
      if record_userstamps and user_model.current_user != nil
        write_attribute(:creator_id, user_model.current_user.id.to_i) if respond_to?(:created_by) and self.created_by.nil?
        write_attribute(:updater_id, user_model.current_user.id.to_i) if respond_to?(:updated_by)
      end
      create_without_user
    end
